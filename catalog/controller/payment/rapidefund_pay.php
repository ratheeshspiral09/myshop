...
class Controllerpaymentrapidefundpay extends Controller {
	protected function index() {
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/rapidefund_pay.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/rapidefund_pay.tpl';
		} else {
			$this->template = 'default/template/payment/rapidefund_pay.tpl';
		}	
	...
	public function send() {
		if ($this->config->get('rapidefund_pay_server') == 'live') {
			$url = 'https://sandbox-ca.rapidefund.com/chatak-pay/pg/transaction/process';
		} elseif ($this->config->get('rapidefund_pay_server') == 'test') {
			$url = 'https://sandbox-ca.rapidefund.com/chatak-pay/pg/transaction/process';		
		}	
		...
		"cardData" => Array(
			...
			"cardNumber" =>str_replace(' ', '', $this->request->post['cc_number']),
			"cardType" =>"VI",
		),

		"entryMode" =>"MANUAL",
		"invoiceNumber" =>$this->session->data['order_id'],
		"merchantAmount" =>$this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false),
		"feeAmount" =>"0",
		"totalTxnAmount" =>$this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false),
		"registerNumber" =>"DROP".$this->session->data['order_id'],
		"orderId" =>$this->session->data['order_id'],
		"merchantId" =>$this->config->get('rapidefund_merchant_id'),
		"merchantName" =>$this->config->get('rapidefund_merchant_name'),
		"terminalId" =>$this->config->get('rapidefund_terminal_id'),
		...
		$data['x_login'] = $this->config->get('rapidefund_pay_login');
		$data['x_tran_key'] = $this->config->get('rapidefund_pay_key');
		$data['x_version'] = '3.1';
		...
		$data['x_method'] = 'CC';
		$data['x_type'] = ($this->config->get('rapidefund_pay_method') == 'capture') ? 'AUTH_CAPTURE' : 'AUTH_ONLY';
		...
		if ($this->config->get('rapidefund_pay_mode') == 'test') {
		...
		curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($ManualSALECardData, '', '&'));
		...
		if (curl_error($curl)) {
			...
			if ($response_info[1] == '1') {
				if (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('rapidefund_pay_hash') . $this->config->get('rapidefund_pay_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false)))) {
					...
					$this->model_checkout_order->update($this->session->data['order_id'], $this->config->get('rapidefund_pay_order_status_id'), $message, false);				
				}
